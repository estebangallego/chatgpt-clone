...
class ChatController extends Controller
{
    ...
    {
        $messages = collect(session('messages', []))->reject(fn ($message) => $message['role'] === 'system')->reverse();
        return view('welcome', [
            'messages' => $messages
        ]);
    }
    ...
    public function store(Request $request): Application|Redirector|RedirectResponse|ApplicationContract
    {
        $messages = $request->session()->get('messages', [
            ['role' => 'system', 'content' => 'You are EstebanGPT - A ChatGPT clone. Answer as concisely as possible.']
        ]);
